...
export default function PerifericosPage() {
  ...
  const totalPages = Math.ceil(filteredPerifericos.length / itemsPerPage) || 1;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const paginatedPerifericos = filteredPerifericos.slice(startIndex, startIndex + itemsPerPage);
      ...
      {loading ? (
        ...
        <div className={styles.tableWrapper}>
          ...
          <div style={{
            ...
            <div>
              Mostrando <strong>{startIndex + 1}</strong> a <strong>{Math.min(startIndex + itemsPerPage, filteredPerifericos.length)}</strong> de <strong>{filteredPerifericos.length}</strong> {filteredPerifericos.length === 1 ? 'registro' : 'registros'}
            </div>
